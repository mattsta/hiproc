...
async fn execute_command_with_tracking(
    ...
    let execution_record = api::ExecutionHistoryCreate {
        ...
        arguments: if args.is_empty() { None } else { Some(args.join(" ")) },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Find { scope, user: user_filter } => {
            let commands = api_client.get_commands("", None, user_filter.as_deref(), scope.as_deref()).await?;
            ...
            if let Some(item) = selected_items.first() {
                ...
                if let (Some(id_part), Some(command_string)) = (parts.get(0), parts.get(1)) {
                        ...
                        if let Ok(command_id) = id_str.trim().parse::<i32>() {
                            ...
                            if let Ok(command) = api_client.execute_command(command_id, &user).await {
                                ...
                                execute_command_with_tracking(
                                    &api_client, &command, &user, &hostname, &cwd, &resolved_command, "find", &[]
                                ).await?;
                                return Ok(());
                            }
                        }
                    ...
                    let resolved_command = secrets::resolve_secrets(command_string.trim())?;
                    let err = exec::Command::new("sh").arg("-c").arg(resolved_command).exec();
        ...
        Commands::Search { query, namespace, scope, user } => {
            let commands = api_client
                .get_commands(&query, namespace.as_deref(), user.as_deref(), scope.as_deref())
                .await
                .context("Failed to search for commands on server")?;
            let mut table = Table::new();
            table.set_header(vec!["ID", "Namespace", "Name", "User", "Hostname", "Scope", "Directory", "Created", "Last Used", "Uses", "Command"]);
            for cmd in commands {
                table.add_row(vec![
                    cmd.id.to_string(),
                    cmd.namespace,
                    ...
                    cmd.last_used_at.map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or_default(),
        ...
        Commands::Namespaces => {
            let namespaces = api_client.get_namespaces().await.context("Failed to get namespaces from server")?;
        ...
        Commands::List { namespace, scope } => {
            ...
            let filtered_commands: Vec<_> = commands.iter()
                .filter(|cmd| {
                    let namespace_match = namespace.as_ref().map_or(true, |ns| &cmd.namespace == ns);
            ...
            let mut table = Table::new();
            table.set_header(vec!["ID", "Namespace", "Name", "Hostname", "Scope", "Directory", "Created", "Last Used", "Uses", "Command"]);
        ...
        Commands::Info { command_id } => {
            let commands = api_client.get_all_user_commands(&user).await?;
            let command = commands.iter().find(|cmd| cmd.id == command_id)
            ...
            table.add_row(vec!["Hostname", command.hostname.as_deref().unwrap_or("N/A")]);
            table.add_row(vec!["Directory", command.cwd.as_deref().unwrap_or("N/A")]);
            table.add_row(vec!["Created", &command.created_at.format("%Y-%m-%d %H:%M:%S").to_string()]);
            table.add_row(vec!["Last Used", &command.last_used_at.map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string()).unwrap_or("Never".to_string())]);
        ...
        Commands::Here { similar, project } => {
            ...
            if project {
                ...
                let project_context = api_client.detect_project_context(project_request).await
                ...
                if !project_context.similar_commands.is_empty() {
                    println!("  Similar Commands: {}", project_context.similar_commands.join(", "));
                }
            } else {
                ...
                let suggestions = api_client.get_suggestions(suggestions_request).await
                    .context("Failed to get context suggestions")?;

                if suggestions.is_empty() {
                    println!("No contextual suggestions found for current directory: {}", cwd);
                } else {
                    println!("Commands relevant to current context ({}):", cwd);
                    let mut table = Table::new();
                    table.set_header(vec!["ID", "Name", "Uses", "Last Used", "Command"]);
                    for cmd in suggestions {
                        let last_used = cmd.last_used_at
        ...
        Commands::Suggest { limit, project_type } => {
            let cwd = env::current_dir()?.to_str().context("Invalid CWD")?.to_string();
            let hostname = gethostname::gethostname().to_str().context("Invalid hostname")?.to_string();
            ...
            if suggestions.is_empty() {
                ...
                let mut table = Table::new();
                table.set_header(vec!["ID", "Namespace", "Name", "Uses", "Last Used", "Command"]);
                for cmd in suggestions {
                    let last_used = cmd.last_used_at
                        .map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string())
                        .unwrap_or("Never".to_string());
        ...
        Commands::Similar { command_id, limit } => {
            let similar_commands = api_client.get_similar_commands(command_id, Some(limit)).await
        ...
        Commands::Analytics { days } => {
            let analytics = api_client.get_execution_analytics(Some(&user), days).await
            ...
            if let Some(avg_per_day) = analytics.get("average_executions_per_day") {
                println!("Average per Day: {:.1}", avg_per_day.as_f64().unwrap_or(0.0));
            }
            
            if let Some(top_commands) = analytics.get("most_used_commands").and_then(|v| v.as_array()) {
                println!("\nMost Used Commands:");
                let mut table = Table::new();
            ...
            if let Some(methods) = analytics.get("execution_methods").and_then(|v| v.as_object()) {
                println!("\nExecution Methods:");
                let mut table = Table::new();
        ...
        Commands::Rename { command_id, new_namespace, new_name } => {
        ...
        Commands::Edit { command_id } => {
            ...
            let commands = api_client.get_all_user_commands(&user).await?;
            let command_to_edit = commands.iter().find(|cmd| cmd.id == command_id)
                .context("Command with specified ID not found or you don't have permission")?;
        ...
        Commands::Delete { command_id } => {
            ...
            let deleted_command = api_client
                .delete_command(command_id, &user)
                .await
                .context("Failed to delete command - command not found or you don't have permission")?;
            
            println!("Deleted command with ID {} ('{}' from namespace '{}')", command_id, deleted_command.name, deleted_command.namespace);
        }
        Commands::Exec { command_id, args } => {
            // Execute command directly by ID
            let command = api_client.execute_command(command_id, &user).await
            ...
            execute_command_with_tracking(
                &api_client, &command, &user, &hostname, &cwd, &resolved_command, "id", &args
            ).await?;
        }
        Commands::Run { name, namespace, scope, args } => {
            ...
            let command = api_client.recall_command_by_name(request).await
            ...
            execute_command_with_tracking(
                &api_client, &command, &user, &hostname, &cwd, &resolved_command, "name", &args
            ).await?;
        }
        Commands::QuickSave { name, namespace } => {
            ...
            if !history_manager.is_available() {
                eprintln!("Shell history integration not available: {}", history_manager.status_description());
                println!("To save a command manually, use: hp save \"<command>\" --name {} --namespace {}", 
                        name, namespace.as_deref().unwrap_or("<namespace>"));
                return Ok(());
            }
            ...
            match history_manager.get_last_command()? {
                Some(command_string) => {
                    ...
                    let cmd = api_client.save_command(new_command).await
                        .context("Failed to save command to server")?;
                    
                    if cmd.is_new {
                        println!("✓ Quick-saved '{}' as '{}' in namespace '{}'", 
        ...
        Commands::Shell { command_parts } => {
            ...
            // Execute using our tracking function
            execute_command_with_tracking(
                &api_client, &saved_command, &user, &hostname, &cwd, &command_string, "shell", &[]
            ).await?;
        }
        Commands::Recall(args) => {
            ...
            if args.len() == 1 || args[0].parse::<i32>().is_ok() {
                if let Ok(command_id) = args[0].parse::<i32>() {
                    ...
                    let templated_command = templating::resolve_arguments(&command.command_string, extra_args)?;
                    let resolved_command = secrets::resolve_secrets(&templated_command)?;
                    ...
                    execute_command_with_tracking(
                        &api_client, &command, &user, &hostname, &cwd, &resolved_command, "id", extra_args
                    ).await?;
            ...
            let command = api_client.recall_command(namespace, name, &user, &hostname, &cwd).await
            ...
            execute_command_with_tracking(
                &api_client, &command, &user, &hostname, &cwd, &resolved_command, "namespace_name", extra_args
            ).await?;
        }
